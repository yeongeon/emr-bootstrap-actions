...
<configuration>"""
  ...
  def create_node(self, parent, node, name, value):
    ...
    SubElement(node, "value").text = value
    parent.append(node)
    return parent
...
class FileUtil:
  ...
  def invoke_run(self, fname, values, hadoop_home):
    self.ln(__file__, "./install_tajo_lib.py", True)
    invoke = InvokeUtil()
...
from xml.etree import ElementTree
import re
from install_tajo_lib import *

targetFile="%s/etc/hadoop/yarn-site.xml"
launcherUtil = LancherUtil()
...
flog.close()
launcherUtil.build()
launcherUtil.start()''' % (hadoop_home,)
...
class LancherUtil:
  ...
  NAME_NODE = None
  NAME_NODE_PORT = 8020
  START_INVOKE_FILE='install-tajo.py'
  TEST_MODE = False
  TEST_DIR = None
  ...
  def parse_opts(self):
    parser = argparse.ArgumentParser(description="Tajo Installer on EMR4")
  ...
  ## Print Help
  def help(self):
    print ('usage : install-tajo.sh [-t|--tar] [-c|--conf] [-l|--lib] [-h|--help] [-e|--env] [-s|--site] [-T|--test-home] [-H|--test-hadoop-home]')
    print (' -t, --tar')
    print ('       The tajo binary Tarball URL.(Optional)')
    print ('       ex) --tar http://apache.mirror.cdnetworks.com/tajo/tajo-0.10.0/tajo-0.10.0.tar.gz')
    print ('       or')
    print ('       --tar s3://[your_bucket]/[your_path]/tajo-0.10.0.tar.gz')
    print (' -c, --conf')
    print ('       The tajo conf directory URL.(Optional)')
    print ('       ex) --conf s3://beta.elasticmapreduce/bootstrap-actions/tajo/template/tajo-0.10.0/c3.xlarge/conf')
    print (' -l, --lib')
    print ('       The tajo third party lib URL.(Optional)')
    print ('       ex) --lib s3://{your_bucket}/{your_lib_dir}')
    print ('       or')
    print ('       --lib http://{lib_url}/{lib_file_name.jar}')
    print (' -v, --tajo-version')
    print ('       The tajo release version.(Optional)')
    print ('       Default: Apache tajo stable version.')
    print ('       ex) x.x.x')
    print (' -h, --help')
    print ('       The help')
    print (' -e, --env')
    print ('       The item of tajo-env.sh(Optional, space delimiter)')
    print ('       ex) --tajo-env.sh "TAJO_PID_DIR=/home/hadoop/tajo/pids TAJO_WORKER_HEAPSIZE=1024"')
    print (' -s, --site')
    print ('       The item of tajo-site.xml(Optional, space delimiter)')
    print ('       ex) --tajo-site.xml "tajo.rootdir=s3://mybucket/tajo tajo.worker.start.cleanup=true tajo.catalog.store.class=org.apache.tajo.catalog.store.MySQLStore"')
    print (' -T, --test-home')
    print ('       The Test directory path(Only test)')
    print ('       ex) --test-home "/home/hadoop/bootstrap_test"')
    print (' -H, --test-hadoop-home')
    print ('       The Test HADOOP_HOME(Only test)')
    print ('       ex) --test-hadoop-home "/home/hadoop"')
  ...
  def init(self, fileUtil, opt):
    ...
    if self.TEST_MODE:
      ...
      master = self.xmlUtil.get_property_value("/usr/lib/hadoop/etc/hadoop/yarn-site.xml", "yarn.resourcemanager.address")
      m = re.search("[^:<]+", master)
      master = m.group()
    ...
    if not self.TAJO_PACKAGE_URI:
      self.TAJO_PACKAGE_URI = "http://d3kp3z3ppbkcio.cloudfront.net/tajo-$TAJO_VERION/tajo-%s.tar.gz" % (self.TAJO_VERSION,)
  ...
  def set_tajo_conf(self):
    ...
    export_lib='''export JAVA_HOME=/usr/lib/jvm/java-1.7.0-openjdk.x86_64
...
''' % (echo_hadoop_home, echo_hadoop_lzo_home)
...
''' % (echo_hadoop_home, echo_hadoop_hdfs_home, echo_hadoop_yarn_home, echo_hadoop_lzo_home)
    ...
    root = tree.getroot()
    root = self.xmlUtil.create_node(root, 'property', 'tajo.master.umbilical-rpc.address', '%s:26001' % (self.TAJO_MASTER,))
    root = self.xmlUtil.create_node(root, 'property', 'tajo.master.client-rpc.address', '%s:26002' % (self.TAJO_MASTER,))
    root = self.xmlUtil.create_node(root, 'property', 'tajo.resource-tracker.rpc.address', '%s:26003' % (self.TAJO_MASTER,))
    root = self.xmlUtil.create_node(root, 'property', 'tajo.catalog.client-rpc.address', '%s:26005' % (self.TAJO_MASTER,))
    ...
    if not self.xmlUtil.get_property_value_from_node(root, 'tajo.rootdir'):
      ...
      if self.TEST_MODE:
        ...
        root = self.xmlUtil.create_node(root, 'property', 'tajo.rootdir', 'hdfs://%s:%d/tajo' % (self.NAME_NODE, self.NAME_NODE_PORT))
  ...
  def third_party_lib(self):
    ...
    if self.LIBRARY_URI:
      ...
      if parser.scheme == "http" or parser.scheme == "https":
        return os.system('curl -o %s/lib/%s %s' % (self.TAJO_HOME, os.path.basename(self.LIBRARY_URI), self.LIBRARY_URI))
      else:
        # Test mode
        if self.TEST_MODE:
